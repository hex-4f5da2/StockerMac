...
enum LiveAPIChecks {
    static func main() async throws {
        ...
        let quotes = try await service.fetchQuotes(for: items, provider: .sina)
        precondition(quotes.count == items.count, "本地 StockDB only returned \(quotes.count) quotes")
        ...
        print("本地 StockDB: " + quotes.map { "\($0.code)=\($0.current)" }.joined(separator: ", "))
    }
}
